...
class wxConfigObj2(wx.Frame):
    ...
    def __init__(self, iniFile, debug=False, actually_write=True):
        ...
        self.SetSize([i*1.1 for i in self.GetSize()]) # why?!?
        self.Show()
    ...
    def make_all_widgets(self):
        """Add widget set for each entry in config to a FlexGridSizer"""
        sizer = wx.FlexGridSizer(len(self.config), 4)
...
if __name__ == '__main__':
    app = wx.App()
    wxConfigObj2(argHandler(), debug=False, actually_write=True)
    app.MainLoop()
